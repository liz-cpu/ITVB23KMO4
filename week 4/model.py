...
def Viterbi(all_possible_states: list[tuple[int, int, str]], observations: list[tuple[int, int]]) -> list[tuple[int, int, str]]:
    ...
    for i in range(len(observations)):
        ...
        if observations[i] == None:
            ...
            for state in states[i]:
                states[i][state] = max([states[i-1][prev_state] * transition_model(prev_state)[
                                       state] * observation_model(state)[prev_state[:2]] for prev_state in states[i-1]])
            continue
        for state in states[i]:
            if i == 0:
                states[i][state] = steady_state_distr[state[2]] * \
                    observation_model(state)[observations[i]]
            else:
                states[i][state] = max([states[i-1][prev_state] * transition_model(prev_state)[
                                       state] * observation_model(state)[observations[i]] for prev_state in states[i-1]])
    ...
    for i in range(len(observations)-1, 0, -1):
        most_likely_state = max([prev_state for prev_state in states[i-1]], key=lambda prev_state: states[i-1]
                                [prev_state] * transition_model(prev_state)[most_likely_state])
