...
Vragen:
a)
    1. Hoeveel (verschillende) permutaties zijn er eigenlijk? Hoe kun je dit zelf
    berekenen? Hou er rekening mee dat (A,A,A,A …) niet kan, je kan immers A
    maar 2 keer kiezen.

        8! / (2! * 2! * 2! * 2!) = 2520 permutaties

    2. Bij het testen is het slim om de test die het meest beperkend is het
    eerst te doen (‘fail fast’). Hoeveel permutaties moeten er worden getest
    (= iteraties) om de eerste en tweede oplossing te vinden?

        Voor de eerste oplossing moesten er 699 iteraties worden gedaan.
        Voor de tweede waren dit 1620.
        1. {0: 'K', 1: 'Q', 2: 'J', 3: 'Q', 4: 'A', 5: 'K', 6: 'J', 7: 'A'},
        2. {0: 'K', 1: 'Q', 2: 'J', 3: 'Q', 4: 'A', 5: 'K', 6: 'A', 7: 'J'}
b)
    Implementeer een betere versie door gebruik te maken van DFS en
    backtracking. Dit kan in ca. 70 regels, waarbij misschien het lastigste is
    de testen correct te implementeren.

    Hoeveel iteraties (recursive calls) zijn nu nodig om de eerste en de
    tweede oplossing te vinden?

        De eerste was gevonden na 11 en de tweede na 12 iteraties.

c)
    Je kan de puzzel ook oplossen door herhaaldelijk arc-consistency toe te
    passen (en wat logisch redeneren).Doe dit op papier.

    Beredeneer dat bord[5] een Heer moet zijn, door te laten zien dat bord[5]
    geen Aas, Vrouw of Boer kan zijn.

    Beredeneer daarna - op eenzelfde manier - dat bord[0] ook een Heer moet zijn.

        Kan worden gevonden in 3c. arc-consistency.pdf
"""
import itertools
...
def is_valid(board: dict[int, chr]) -> bool:
    """
    Checks if the current board state is valid.

    :param board: a dictionary with the cell number as key and the card as value
    :type board: dict[int, chr]
...
def test() -> None:
    ...
    print('f ', is_valid({0: 'J', 1: 'K', 2: 'Q',
          3: 'Q', 4: 'J', 5: 'K', 6: 'A', 7: 'A'}))
    print('f ', is_valid({0: 'J', 1: 'J', 2: 'Q',
          3: 'Q', 4: 'K', 5: 'K', 6: 'A', 7: 'A'}))
    print('t ', is_valid({0: '.', 1: '.', 2: '.',
          3: '.', 4: '.', 5: '.', 6: '.', 7: '.'}))
    print('t ', is_valid({0: 'J', 1: '.', 2: '.',
          3: '.', 4: '.', 5: '.', 6: '.', 7: '.'}))
    print('f ', is_valid({0: '.', 1: '.', 2: '.', 3: 'J',
          4: 'J', 5: 'A', 6: 'J', 7: 'J'}))  # [1]
    print('f ', is_valid({0: 'J', 1: '.', 2: '.', 3: '.',
          4: 'J', 5: 'K', 6: 'J', 7: 'Q'}))  # [3]
    print('t ', is_valid({0: '.', 1: 'Q', 2: '.', 3: '.',
          4: 'Q', 5: 'J', 6: '.', 7: '.'}))  # [3]
    print('f ', is_valid({0: 'Q', 1: '.', 2: '.', 3: 'K',
          4: '.', 5: '.', 6: '.', 7: '.'}))  # [3]
    print('f ', is_valid({0: '.', 1: 'A', 2: 'Q', 3: '.',
          4: '.', 5: 'Q', 6: '.', 7: '.'}))  # [4]
    print('f ', is_valid({0: '.', 1: '.', 2: '.', 3: '.',
          4: 'J', 5: 'J', 6: '.', 7: '.'}))  # [5]
    print('f ', is_valid({0: '.', 1: '.', 2: '.', 3: '.',
          4: '.', 5: 'Q', 6: '.', 7: 'Q'}))  # [5]
    print('t ', is_valid({0: 'Q', 1: 'Q', 2: '.',
          3: '.', 4: '.', 5: '.', 6: '.', 7: '.'}))


def brute_force(board: dict[int, chr], cards: list[chr]) -> Generator[dict[int, chr], None, None]:
    """
    Brute force all possible permutations of the cards and check if the board
    is valid.

    :param board: The board to brute force.
    :type board: dict[int, chr]
    :param cards: The cards on the stack.
    :type cards: list[chr]
    :yield: A valid board.
    :rtype: Generator[dict[int, chr], None, None]
    """
...
def dfs_backtrack(board: dict[int, chr], cards: list[chr]) -> Generator[dict[int, chr], None, None]:
    """
