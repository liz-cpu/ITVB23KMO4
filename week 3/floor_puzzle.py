...
def get_valid_permutations(num_floors: int, persons: list[chr]) -> Generator[dict[chr, int], chr, None]:
    # get a list of dictionaries with all possible configurations
    configurations = [dict(zip(persons, configuration))
                      for configuration in itertools.permutations(range(num_floors), len(persons))]
...
def is_valid_configuration(configuration: dict, num_floors: int) -> bool:
    """
...
print([sorted(valid_config, key=valid_config.get)
      for valid_config in valid_configs])

"""
Op de begaande grond woont Joep
Op de eerste verdieping woont Marja,
en daarboven woont Niels.
Op de derde verdieping woont Loes en op
de hoogste verdieping woont Erik.
"""
